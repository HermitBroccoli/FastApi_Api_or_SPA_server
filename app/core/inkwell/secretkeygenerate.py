        ...
        with open(env_file_path, "w") as env_file:
            for line in lines:
                if line.startswith("KEY="):
                    ...
                    env_file.write(f"KEY=base64:{secret_key}\n")
                else:
                    # В противном случае, оставить строку без изменений
                    env_file.write(line)

    else:
        # Если файла .env не существует, создать его и записать в него сгенерированный секретный ключ
        with open(env_file_path, "w") as env_file:
...
def main():
    ...
    secret_key = create_or_load_secret_key()
